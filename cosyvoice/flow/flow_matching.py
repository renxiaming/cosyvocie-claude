    ...
    def solve_euler(self, x, t_span, mu, mask, spks, cond):
        ...
        for step in range(1, len(t_span)):
            ...
            if torch.npu.is_available() and self.flow_om_static and target_len is not None:
                ...
                # 2. 如果当前长度不等于档位，进行手动补零对齐
                if curr_len < target_len:
                    pad_len = target_len - curr_len
                    # 只有最后一位维度需要补零，(0, pad_len) 表示在最后一个维度右侧补 pad_len 个 0
                    # 注意：Numpy 的 pad 和 Torch 的 F.pad 略有不同，这里建议直接处理 numpy 或 torch
                    # 为了兼容性，我们直接对 feed 列表里的 numpy 数组做处理
                    processed_feed = []
                    for tensor in feed_list:
                        arr = tensor.cpu().detach().numpy().astype(np.float32)
                        if len(arr.shape) == 3: # 处理 x, mask, mu, cond
                            arr = np.pad(arr, ((0,0), (0,0), (0, pad_len)), mode='constant')
                        processed_feed.append(arr)
                    feed = processed_feed
                else:
                ...
                # 4. 取回结果并截断多余的 Padding
                dphi_dt = torch.from_numpy(dphi_dt[0]).npu()
                if curr_len < target_len:
                    dphi_dt = dphi_dt[:, :, :curr_len]
            # --- 修改结束 ---
