...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for _ in range(args.warm_up_times):
            next(cosyvoice.inference_sft_streaming(prompt_txt, '中文女', stream=args.stream))
        print('warm up end')
        # import ipdb;ipdb.set_trace()
        for _ in range(args.infer_count):
            for i, j in enumerate(cosyvoice.inference_sft_streaming(prompt_txt, '中文女', stream=args.stream)):
